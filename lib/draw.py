        ...
        for y in range(render_h_min, render_h_max):
            ...
            if is_visible:
                ...
                if map_to_draw[x][y].block_path == True:
                    globalvars.SURFACE_MAP.blit(globalvars.ASSETS.wall_dict[tile_assignment],
                                                (x * constants.CELL_WIDTH, y * constants.CELL_HEIGHT))
                else:
                    globalvars.SURFACE_MAP.blit(globalvars.ASSETS.S_FLOOR[0],
                                                (x * constants.CELL_WIDTH, y * constants.CELL_HEIGHT))

            elif map_to_draw[x][y].explored:

                if map_to_draw[x][y].block_path == True:
                    globalvars.SURFACE_MAP.blit(globalvars.ASSETS.wall_ex_dict[tile_assignment],
                                                (x * constants.CELL_WIDTH, y * constants.CELL_HEIGHT))
                else:
                    globalvars.SURFACE_MAP.blit(globalvars.ASSETS.S_FLOOREXPLORED[0],
                                                (x * constants.CELL_WIDTH, y * constants.CELL_HEIGHT))


def draw_text(display_surface, text_to_display, T_coords, text_color, font=constants.MENU_FONT, back_color=None,
              center=False):
...
def draw_debug():
    draw.draw_text(globalvars.SURFACE_MAIN, "fps: " + str(int(globalvars.CLOCK.get_fps())), font=constants.MENU_FONT,
                   T_coords=(0, 0), text_color=constants.COLOR_RED)
...
def draw_messages():
    ...
    for message, color in to_draw:
        draw.draw_text(globalvars.SURFACE_MAIN, message, (0, start_y + (i * text_height)), color,
                       constants.MESSAGE_FONT, constants.COLOR_BLACK)
        i += 1
...
def draw_tile_rect(coords, tile_color=None, tile_alpha=None, mark=None):
    ...
    if mark:
        draw.draw_text(new_surface, mark, T_coords=(constants.CELL_WIDTH / 2, constants.CELL_HEIGHT / 2),
                       text_color=constants.COLOR_BLACK, font=constants.CURSOR_TEXT, center=True)
